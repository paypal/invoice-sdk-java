...
public class SearchInvoicesServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		/*
		 * SearchInvoicesRequest which takes mandatory params: # `Request
		 * Envelope` - Information common to each API operation, such as the
		 * language in which an error message is returned. # `Merchant Email` -
		 * Email address of invoice creator. # `SearchParameters` - Parameters
		 * constraining the search. # `Page` - Page number of result set,
		 * starting with 1. # `Page Size` - Number of results per page, between
		 * 1 and 100.
		 */
		...
		/*
		 * (Optional) Currency used for lower and upper amounts. It is required
		 * when you specify lowerAmount or upperAmount.
		 */
		...
		if (request.getParameter("invoiceStartDate") != ""
			...
			if (request.getParameter("invoiceStartDate") != "")
				invoiceDate.setStartDate(request
						.getParameter("invoiceStartDate") + "T00:00:00.000Z");
		...
		/*
		 * (Optional) Invoice amount search. It specifies the smallest amount to
		 * be returned. If you pass a value for this field, you must also pass a
		 * currencyCode value.
		 */
		...
		/*
		 * (Optional) Indicates whether the invoice was created by the website
		 * or by an API call. It is one of the following values:
		 * 
		 * Web – The invoice was created on paypal.com. API – The invoice was
		 * created by an Invoicing Service API call.
		 */
		...
		if (request.getParameter("paymentStartDate") != ""
			...
			if (request.getParameter("paymentStartDate") != "")
				paymentDate.setStartDate(request
						.getParameter("paymentStartDate") + "T00:00:00.000Z");
		...
		/*
		 * (Optional) Invoice amount search. It specifies the largest amount to
		 * be returned. If you pass a value for this field, you must also pass a
		 * currencyCode value.
		 */
		...
		try {
			...
			// wrapper object
			SearchInvoicesResponse resp = null;
			if (cred != null) {
				resp = invoiceSrvc.searchInvoices(req, cred);
			} else {
				resp = invoiceSrvc.searchInvoices(req);
			}
			String lastReq = null;
			if (resp != null) {
